...
import { getSpecialBlindInfo, createSpecialBlindOrder } from '@/api/blindMarket';
...
const getProductBuy = async() => {
  ...
  const [_, res] = await to(createSpecialBlindOrder(data));

  if(res) {
    if(data.type == 1 || data.type == 2) {
      router.push(`/one/pay?order_id=${res.order_id}&blind_price=${res.blind_price}`);
    }
  }
}
</script>
      ...
      <div class="content-product-image">
        <div class="content-title-list">
          <div class="content-title-number">
            <span class="content-title">1元盲盒</span>
          </div>
