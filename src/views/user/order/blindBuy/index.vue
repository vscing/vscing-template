...
import { getBlindOrderList, orderCancel } from '@/api/blindMarket';
...
const onLoad = async () => {
  const [_, res] = await to(getBlindOrderList({
    page: page.value,
    payment_status: arr[active.value]
  }));
  if (res) {
    list.value = res.list || []
    ...
    <ul class="product-list">
      <li class="product-item" v-for="item in list" :key="item.id">
        ...
        <div class="product-item-info">
          ...
          <p class="setup" v-if="item.payment_status == 10">如已付款请不要点击取消，否则盲盒不到账</p>
        </div>
      </li>
    </ul>
