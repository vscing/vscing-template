...
import { getBlindList } from '@/api/blind';
...
const onLoad = async () => {
  const [_, res] = await to(getBlindList({
    page: page.value,
    type: active.value
  }));
  if (res) {
...
const goDetail = (id: number) => {
  router.push('/blind/detail?id='+id)  
}

</script>

<template>
  <VantNavBar class="nav-bar" safe-area-inset-top fixed title="我的盲盒" left-arrow @click-left="onClickLeft" />
    ...
    <ul class="product-list">
      <li class="product-item" v-for="item in list" :key="item.id" @click="goDetail(item.id)">
        ...
        <div class="product-item-info">
          <h2>食艺术盲盒</h2>
          <p class="product-item-price">￥{{ item.blind_price }}</p>
  ...
  <VantEmpty v-else class="empty" :image="Images.order" :description="`暂无盲盒`" />
</template>
